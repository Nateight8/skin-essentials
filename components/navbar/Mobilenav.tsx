...
const MobileNav = ({ toggleNav, isOpen }: Props) => {
  ...
  return (
    <div className="" aria-modal="true" role="dialog">
      <div
        className={`absolute top-0 left-0 h-screen py-10 z-50 bg-stone-300 flex flex-col max-w-xs w-full focus:outline-none transform transition ease-in-out duration-300 
      ...
      <div
        className={`  z-40 bg-slate-900 opacity-75 fixed overflow-y-hidden top-0 left-0 h-screen w-screen z-10 duration-500 delay-100 ease-in-out ${
          isOpen ? "translate-x-0 " : "-translate-x-full "
        }`}
      ></div>
    </div>
